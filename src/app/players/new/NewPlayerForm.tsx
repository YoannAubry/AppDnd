    ...
    <form action={createPlayerAction} className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="col-span-2">
          ...
          <input name="name" required className="w-full theme-input" placeholder="Aragorn" />
        </div>
        <div>
          <label className="block text-sm text-[var(--text-muted)] mb-1 font-bold">Joueur (IRL)</label>
          <input name="playerName" className="w-full theme-input" placeholder="Pierre" />
        </div>
        <div>
          <label className="block text-sm text-[var(--text-muted)] mb-1 font-bold">Race</label>
          <input name="race" className="w-full theme-input" placeholder="Humain" />
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="col-span-1">
          ...
          <input name="class" className="w-full theme-input" placeholder="Rôdeur" />
        </div>
        <div>
          <label className="block text-sm text-[var(--text-muted)] mb-1 font-bold">Niveau</label>
          <input name="level" type="number" defaultValue={1} className="w-full theme-input text-center" />
        </div>
      </div>
      ...
      <div className="bg-[var(--bg-input)] p-4 rounded border border-[var(--border-main)]">
        ...
        <div className="grid grid-cols-3 gap-4">
          <div>
            ...
            <input name="hpMax" type="number" required className="w-full theme-input text-center" placeholder="12" />
          </div>
          <div>
            <label className="block text-xs text-blue-500 mb-1 text-center font-bold">CA</label>
            <input name="ac" type="number" required className="w-full theme-input text-center" placeholder="14" />
          </div>
          <div>
            <label className="block text-xs text-[var(--text-muted)] mb-1 text-center font-bold">INIT BONUS</label>
            <input name="initBonus" type="number" defaultValue={0} className="w-full theme-input text-center" placeholder="+2" />
          </div>
        </div>
      </div>

      <div className="pt-6 flex justify-end gap-4 border-t border-[var(--border-main)]">
        <Link href="/players" className="px-4 py-2 text-[var(--text-muted)] hover:text-[var(--text-main)]">Annuler</Link>
        <button type="submit" className="theme-btn-primary">Enrôler</button>
      </div>
