...
export default async function LocationPage(props: { params: Promise<{ id: string }> }) {
  ...
  if (!location) return <div className="p-20 text-center text-[var(--text-muted)]">Lieu introuvable</div>
    ...
    <div className="min-h-screen bg-[var(--bg-main)] text-[var(--text-main)] font-serif pb-20">
      ...
      <div className="relative h-64 md:h-80 bg-black overflow-hidden shadow-xl border-b-4 border-[var(--border-accent)]">
        ...
        <div className="absolute bottom-8 left-8 md:left-12 z-10">
          <h1 className="text-5xl font-bold text-white drop-shadow-lg mb-2 tracking-wide">{location.name}</h1>
        </div>
        
        <Link href="/locations" className="absolute top-6 left-6 bg-black/40 hover:bg-black/60 text-white px-4 py-2 rounded-full backdrop-blur-sm transition font-sans text-sm font-bold z-20 border border-white/10">
          ← Carte
        </Link>
      </div>
      ...
      <div className="max-w-5xl mx-auto px-6 md:px-12 mt-12 grid md:grid-cols-3 gap-12">
        ...
        <div className="md:col-span-2 space-y-8">
          <div className="prose prose-lg prose-invert text-[var(--text-main)]">
            {location.description ? (
              <PortableText value={location.description} />
            ) : (
              <p className="italic text-[var(--text-muted)]">Aucune description disponible.</p>
            )}
          </div>
        </div>
            ...
            <div className="space-y-3">
              {location.npcs?.map((npc: any) => (
                <Link href={`/npcs/${npc._id}`} key={npc._id} className="flex items-center gap-3 group p-2 -mx-2 rounded hover:bg-[var(--bg-input)] transition">
                  <div className="w-10 h-10 rounded-full bg-[var(--bg-input)] overflow-hidden border border-[var(--border-main)]">
                    {npc.image ? (
                        // eslint-disable-next-line @next/next/no-img-element
                  ...
                  <div>
                    <div className="font-bold text-[var(--text-main)] group-hover:text-[var(--accent-primary)] transition">{npc.name}</div>
                    <div className="text-xs text-[var(--text-muted)]">{npc.role}</div>
                  </div>
                </Link>
              ))}
            ...
            <div className="space-y-2">
              {location.monsters?.map((monster: any) => (
                <Link href={`/bestiary/${monster.slug}`} key={monster._id} className="block bg-[var(--bg-card)] border border-[var(--border-main)] rounded px-3 py-2 text-sm text-[var(--accent-primary)] font-bold hover:bg-[var(--bg-input)] hover:border-[var(--accent-primary)] transition shadow-sm flex justify-between items-center group">
                  <span className="group-hover:translate-x-1 transition-transform">{monster.name}</span>
                  <span className="text-[var(--text-muted)] text-xs font-normal">CR {monster.cr}</span>
                </Link>
              ))}
