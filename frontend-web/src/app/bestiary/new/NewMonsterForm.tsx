...
export default function NewMonsterForm() {
  ...
  return (
    <form action={createMonsterAction} className="space-y-6 md:space-y-8">
      
      {/* IDENTITÉ (Responsive 1 -> 2 cols) */}
      ...
      {/* STATS DE BASE (Reste 3 cols mais gap réduit mobile) */}
      <div className="grid grid-cols-3 gap-2 md:gap-4 bg-[var(--bg-input)] p-3 md:p-4 rounded-lg border border-[var(--border-main)]">
         <div>
            <label className="block text-xs md:text-sm text-green-500 font-bold mb-1 text-center">PV</label>
            <input name="hp" required className="w-full theme-input text-center px-1" placeholder="7 (2d6)"/>
         </div>
         <div>
            <label className="block text-xs md:text-sm text-blue-500 font-bold mb-1 text-center">CA</label>
            <input name="ac" type="number" required className="w-full theme-input text-center px-1" placeholder="15"/>
         </div>
         <div>
            <label className="block text-xs md:text-sm text-yellow-500 font-bold mb-1 text-center">Vit</label>
            <input name="speed" className="w-full theme-input text-center px-1" placeholder="9m"/>
         </div>
      </div>

      {/* CARACTÉRISTIQUES (Responsive 3 -> 6 cols) */}
      <div>
      ...
      <div className="pt-8 flex flex-col-reverse md:flex-row justify-end gap-4 border-t border-[var(--border-main)] mt-8">
        <Link href="/bestiary" className="px-6 py-3 rounded-lg text-[var(--text-muted)] hover:text-[var(--text-main)] hover:bg-[var(--bg-input)] transition text-center">Annuler</Link>
        <button type="submit" className="theme-btn-primary w-full md:w-auto flex items-center justify-center gap-2">
